...
`L_Cathedral_0908_1630.mp4`（`<stem>_<月日>_<时分>`），默认 8K / H.264 / 200 Mbps / 30 fps。

**自动命名每次跑都是新名字，所以它不会续跑。** 时间戳只到分钟（用户 2026-09-08 指定的宽度），
所以同一分钟内的第二次运行会自动加后缀 `_2` —— 而不是接着写第一次的产物。 中途 Ctrl-C 想接着跑，
...
| `--out` | 程序目录下自动命名 | 视频模式是 .mp4 路径，帧模式是**目录**。自动命名是 `<stem>_<月日>_<时分>`，**每次跑都是新名字，所以不会续跑**（同一分钟内撞名就加 `_2`）；要续跑就把上次那条路径显式传进来 |
| `--stem` | | 目录里有多个文件 stem 时选择要处理的集合 |
...
|---|---|---|
| 产物 | 一个 `<stem>_<月日>_<时分>.mp4` | 一个 `<stem>_<月日>_<时分>/` 目录，里面是 `<stem>_S_<帧号>.png` |
...
| 命令 | 包体 | 目标机器要有 |
|---|---|---|
| `build_exe.py --gpu` | 文件夹 542 MiB → **1424 MiB** | NVIDIA 驱动，别的都不用（含 50 系） |
