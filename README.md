...
8K 实测 **2.0 s/帧**（上一代 46.85 s）。下一步 P3：收尾与母版 sink。
...
- **ffmpeg + ffprobe**，带 `libx264` 和 `libx265`（放在程序同级目录，或在 PATH 上；
  程序先找同级目录）
- 内存：8K 渲染时 Python 侧约 3 GB；**当前版本 ffmpeg 会另占约 16 GB**（x264 自动线程在 8K 上的
  缓冲池，正在修，见 ROADMAP P2 收尾清单）。修好前不要在小内存机器上跑 8K
...
| `--size` | `8k` | `8k` = 7680×3840，`4k` = 4096×2048 |
...
```powershell
.venv/Scripts/python.exe -m pytest              # 186 个测试；几何/目录发现的测试不依赖真实数据
