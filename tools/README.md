...
| `copy_frames.py` | 不验证任何结论，是**搬数据**用的：把一个来源目录里每个相机的前 N 帧（默认 100）按原目录结构拷到别处，结果本身就是合法来源目录；`--common` 只拷各相机共有的帧。**纯标准库、Python 3.9 即可**，能直接在没装环境的机器上跑 | 看数据量 |
| `harmonise_probe.py` | P9「光度和谐化」—— 每帧校正前后的指标 A、成品改动了多少、最不一致区域的前后对比截图；`--temporal N` 连续 N 帧看校正量帧间变化（防闪烁） | 每帧约 20 s，30 帧时域约 1 分钟 |
| `rig_probe.py` | §9「指标 A 的两种失败签名」—— 按重叠一致性扫 FOV / 环仰角 / 镜像，不需要参考全景就能判断一套 tile 到底是按什么 rig 渲的；E:\22 上必须找回 90° | 约 2 分钟 |
| `align_probe.py` | §3「装配不是每一份数据都成立」—— 逐对相机在重叠区做相位相关，量它们把同一方向画在了差多少像素的位置；共节点、角度准的 rig 给 0.00 px，`E:\0910B` 给 2.24 px | 约 1 分钟 |
| `blend_probe.py` | §3「分频混合」—— 在重叠区找孤立小物体，量每种混合方式保住了多少对比度（默认 / 羽化 8 / `--two-band`）；100% 是画出它的那台相机 | 约 2 分钟 |
